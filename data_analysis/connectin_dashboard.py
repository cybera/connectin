...
app.layout = html.Div(children=[
 ...
 html.Div([
      ...
      dcc.RadioItems(
          id="radio_table",
            options=[
                {'label': 'Download speed', 'value': 'SPEEDTEST_DOWNLOAD'},
                {'label': 'Upload speed', 'value': 'SPEEDTEST_UPLOAD'},
                {'label': 'Ping latency', 'value': 'SPEEDTEST_PING'},
            ],
...
def update_slider_example_marks(input):
    if input=="speedtest":
        ...
        query_tail=" AND PROVIDER='iperf'"
    query_upload_last = "SELECT LAST(UPLOAD), time FROM SPEEDTEST_UPLOAD WHERE UPLOAD>0"+query_tail+";"
    result_stats=get_dataframe_from_influxdb(client_df=client_df,query_influx=query_upload_last,table_name='SPEEDTEST_UPLOAD')
    res1=result_stats["time"].iloc[0]

    query_upload_first = "SELECT FIRST(UPLOAD), time FROM SPEEDTEST_UPLOAD WHERE UPLOAD>0"+query_tail+";"
    result_stats=get_dataframe_from_influxdb(client_df=client_df,query_influx=query_upload_first,table_name='SPEEDTEST_UPLOAD')
...
def update_dropdown_devices(method, dates, marks):
    ...
    query_devices = "SELECT * FROM SPEEDTEST_UPLOAD WHERE UPLOAD>0 AND time >= '"+date1+"' AND time < '"+date2+"'"+query_tail+";"
    result_dev=get_dataframe_from_influxdb(client_df=client_df,query_influx=query_devices,table_name='SPEEDTEST_UPLOAD')
    device_numbers=[]
...
def create_summary_graph(table,summary,devices_d,method, dates, marks,graph_type):
    ...
    if  not result.empty:
        ...
        u_line=False
        ytitle="Mbps"
        if table=="SPEEDTEST_DOWNLOAD":
            d_line=True
        elif table=="SPEEDTEST_UPLOAD":
            u_line=True
        else:
            ytitle="Miliseconds"    
...
def create_graph_table(table,summary,devices_d,method, dates, marks,graph_type):
    ...
    if  not result.empty:
        ...
        if not table=="SPEEDTEST_PING":
            if table=="SPEEDTEST_UPLOAD":
                treshold=10
            else :
                treshold=50
